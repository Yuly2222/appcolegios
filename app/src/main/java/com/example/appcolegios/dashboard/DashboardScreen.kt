...
private fun ChartSection(state: DashboardState) {
    Card(
        ...
        Column(Modifier.fillMaxWidth().padding(16.dp)) {
            // Título único para la sección visual
...
private fun PieChartSection(state: DashboardState) {
    Card(
        ...
        Column(Modifier.fillMaxWidth().padding(16.dp)) {
            // Título único (usar string resource)
            ...
            // Secciones: estudiantes, docentes y otros (sin duplicados)
            val sections = listOf(
            ...
            SimplePieChart(
                values = sections.map { it.second },
                colors = listOf(MaterialTheme.colorScheme.tertiary, MaterialTheme.colorScheme.secondary, MaterialTheme.colorScheme.primaryContainer),
                labels = sections.map { it.first },
                total = total
            )
        }
    }
...
private fun SimplePieChart(values: List<Float>, colors: List<Color>, labels: List<String>, total: Float) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
        Box(contentAlignment = Alignment.Center) {
            ...
            // Mostrar total en el centro del gráfico (usando el string resource)
            Text(text = stringResource(R.string.total_count, total.toInt()), style = MaterialTheme.typography.bodyMedium)
        }

        Spacer(Modifier.height(12.dp))

        // Leyenda con color y porcentaje (una sola línea por elemento)
