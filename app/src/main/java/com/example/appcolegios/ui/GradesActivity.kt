     ...
     override fun onCreate(savedInstanceState: Bundle?) {
         ...
         lifecycleScope.launch {
             profileVm.student.collectLatest { result ->
                 ...
                 // Resolver nombre: priorizar nombre del student, si no usar displayName del auth
                 val resolvedName = when {
                     !student?.nombre.isNullOrBlank() -> student?.nombre
                     else -> auth.currentUser?.displayName ?: ""
                 }
                 loadGradesForTarget(targetId, resolvedName)
             }
         }
     }
